...
async def ai_tts_gemini(req: GeminiTtsReq, user=Depends(get_current_user)):
    ...
    await rate_limiter.check_rate_limit(f"tts_gemini:{user['id']}", max_per_min=60)
...
async def ai_tts_fish(req: FishTtsReq, user=Depends(get_current_user)):
    ...
    await rate_limiter.check_rate_limit(f"tts_fish:{user['id']}", max_per_min=60)
...
async def ai_tts_kokoro(req: KokoroTtsReq, user=Depends(get_current_user)):
    """
    ...
    """
    await rate_limiter.check_rate_limit(f"tts_kokoro:{user['id']}", max_per_min=60)
...
async def ai_tts_edge(req: EdgeTtsReq, user=Depends(get_current_user)):
    """
    ...
    """
    await rate_limiter.check_rate_limit(f"tts_edge:{user['id']}", max_per_min=60)
