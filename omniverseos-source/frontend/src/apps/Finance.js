...
export default function Finance() {
  ...
  const byCat   = cats.map((cat) => ({ name: cat, amount: txns.filter((t) => t.category === cat && t.type === "expense").reduce((s, t) => s + t.amount, 0) }));

  return (
    <div className="flex flex-col md:grid md:grid-cols-3 h-full text-white" data-testid="finance-app">
      {/* Main panel */}
      <div className="md:col-span-2 p-4 sm:p-5 overflow-y-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-5">
          {[
            ...
            <div key={s.l} className="glass-light rounded-xl p-4">
              ...
              <div className="font-heading text-2xl font-bold" style={{ color: s.c }}>
                ₹{s.v.toFixed(2)}
              </div>
            </div>
          ))}
        ...
        <div className="space-y-1">
          {txns.map((t) => (
            <div key={t.id} className="flex items-center justify-between glass-light rounded-lg px-3 py-2">
              ...
              <div className="flex items-center gap-2 ml-2 flex-shrink-0">
                <span className={`font-mono font-bold text-sm ${t.type === "income" ? "text-[#39FF14]" : "text-[#FF003C]"}`}>
                  {t.type === "income" ? "+" : "-"}₹{t.amount.toFixed(2)}
                </span>
                <button onClick={() => del(t.id)} className="text-slate-600 hover:text-[#FF003C]">
          ...
          {txns.length === 0 && (
            <div className="text-center text-slate-500 text-sm py-6">No transactions yet</div>
          )}
        </div>
      </div>

      {/* Sidebar: add transaction */}
      <div className="border-t md:border-t-0 md:border-l border-white/10 p-4 sm:p-5 overflow-y-auto">
        ...
        <div className="space-y-2">
          ...
          <input
            ...
            className="input-cyber"
          />
          <button data-testid="txn-add" onClick={add} className="neon-btn primary w-full justify-center">
            Add
          </button>
        </div>
      </div>
