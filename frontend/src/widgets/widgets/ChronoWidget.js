...
// ── Parse raw Open-Meteo hourly arrays → [{time, temp, code}] ────────────────
// Keeps only hours from now onwards, max 120 hours (5 days)
function parseHourly(hourlyJson) {
  if (!hourlyJson?.time) return [];
  const now = new Date();
  const nowH = new Date(now.getFullYear(), now.getMonth(), now.getDate(), now.getHours());
  return hourlyJson.time
    .map((t, i) => ({
      time: new Date(t),
      temp: Math.round(hourlyJson.temperature_2m[i]),
      code: hourlyJson.weathercode[i],
    }))
...
async function fetchByCoords(lat, lon) {
  const [meteoRes, geoRes] = await Promise.all([
    fetch(
      ...
      `&current=${METEO_CURRENT}&hourly=${METEO_HOURLY}${METEO_SUFFIX}`
    ),
    fetch(
...
function SecondsArc({ seconds }) {
  ...
  return (
    <svg width={64} height={64} style={{ position: "absolute", top: 0, left: 0, pointerEvents: "none" }}>
      ...
      <circle cx={cx} cy={cy} r={r} fill="none" stroke="#00F0FF" strokeWidth={2}
...
function Brackets({ color = "#00F0FF", size = 10, thickness = 1.5, opacity = 0.4 }) {
...
function HexTicker({ seed }) {
  ...
  return (
    <span style={{ fontFamily: "monospace", fontSize: 7.5, color: "rgba(0,240,255,0.22)", letterSpacing: "0.08em" }}>
      {vals.join(" ")}
    </span>
  );
}
...
function ForecastStrip({ hourly, onClose }) {
  ...
  for (const h of hourly) {
    ...
    if (ds !== currentDay) {
      days.push({ label: dayLabel(h.time), hours: [] });
      currentDay = ds;
    }
    days[days.length - 1].hours.push(h);
  }

  // Min/max temp across all hours for bar scaling
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        <span style={{ fontFamily: "monospace", fontSize: 7.5, letterSpacing: "0.2em", color: "rgba(0,240,255,0.5)" }}>
          ATMO.FORECAST // 5-DAY HOURLY
        </span>
        <button
          onClick={onClose}
          style={{
            background: "transparent", border: "1px solid rgba(0,240,255,0.2)",
            borderRadius: 5, color: "rgba(0,240,255,0.5)", fontSize: 8,
            fontFamily: "monospace", padding: "2px 7px", cursor: "pointer", letterSpacing: "0.1em",
          }}
          onMouseEnter={e => { e.currentTarget.style.color = "#00F0FF"; e.currentTarget.style.borderColor = "rgba(0,240,255,0.5)"; }}
          onMouseLeave={e => { e.currentTarget.style.color = "rgba(0,240,255,0.5)"; e.currentTarget.style.borderColor = "rgba(0,240,255,0.2)"; }}
        >
          ✕ CLOSE
        </button>
      </div>
        ...
        <div style={{ display: "flex", alignItems: "flex-start", gap: 0, height: "100%" }}>
          {days.map((day, di) => (
            <div key={di} style={{ display: "flex", flexDirection: "column", flexShrink: 0 }}>
              ...
              <div style={{
                padding: "6px 8px 4px",
                fontFamily: "monospace", fontSize: 7.5, letterSpacing: "0.15em",
                color: di === 0 ? "#00F0FF" : "rgba(255,255,255,0.3)",
                borderBottom: `1px solid ${di === 0 ? "rgba(0,240,255,0.25)" : "rgba(255,255,255,0.06)"}`,
                whiteSpace: "nowrap",
                background: di === 0 ? "rgba(0,240,255,0.04)" : "transparent",
              }}>
                {day.label}
              </div>
              ...
              <div style={{ display: "flex", flex: 1 }}>
                {day.hours.map((h, hi) => {
                  const c   = wmo(h.code);
                  const pct = (h.temp - minTemp) / tempRange; // 0–1
                  // Colour the temp from cool (violet) → neutral (cyan) → warm (amber)
                  const tempColor = pct < 0.33
                    ? "#A78BFA"
                    : pct < 0.66
                      ? "#00F0FF"
                      : "#FCEE09";
                  const hourLabel = h.time.getHours().toString().padStart(2, "0") + ":00";
                  ...
                  return (
                    ...
                    >
                      {/* Hour */}
                      <div style={{ fontFamily: "monospace", fontSize: 7.5, color: "rgba(255,255,255,0.3)" }}>
                        {hourLabel}
                      </div>
                      ...
                      <i
                        ...
                        style={{
                          fontSize: 13,
                          color: c.color,
                          filter: `drop-shadow(0 0 4px ${c.color}60)`,
                        }}
                      />
                      ...
                      <div style={{
                        ...
                        <div style={{
                          ...
                          borderRadius: 2,
                          boxShadow: `0 0 4px ${tempColor}80`,
      ...
      <div style={{
        ...
        {[["#A78BFA", `${minTemp}° min`], ["#00F0FF", "mid"], ["#FCEE09", `${maxTemp}° max`]].map(([col, lbl]) => (
...
export default function ChronoWidget() {
  ...
  const [is24h,       setIs24h]       = useState(() => {
    try { return localStorage.getItem(LS_FMT) !== "false"; } catch { return true; }
  });
  ...
  const loadWeather = useCallback(async () => {
    ...
    if (city) {
      try { setWeather(await fetchByCity(city)); setWxStatus("ok"); return; } catch {}
    }
    if (navigator.geolocation) {
      await new Promise(resolve => {
        navigator.geolocation.getCurrentPosition(
          async pos => {
            try { setWeather(await fetchByCoords(pos.coords.latitude, pos.coords.longitude)); setWxStatus("ok"); }
            catch { setWxStatus("error"); }
            resolve();
          },
          () => {
            fetchByCity(DEFAULT_CITY)
              .then(d => { setWeather(d); setWxStatus("ok"); })
              .catch(() => setWxStatus("error"))
              .finally(resolve);
          },
          { timeout: 6000 }
      ...
      try { setWeather(await fetchByCity(DEFAULT_CITY)); setWxStatus("ok"); }
      catch { setWxStatus("error"); }
    }
  }, []);
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      setWxStatus(weather ? "ok" : "error");
    }
  };

  const clearCity = () => {
    localStorage.removeItem(LS_CITY);
    setSearching(false); setSearchInput(""); setSearchErr("");
    loadWeather();
  };

  const toggle24h = () => {
    setIs24h(v => { const next = !v; try { localStorage.setItem(LS_FMT, String(next)); } catch {} return next; });
  };
  ...
  return (
    <div className="w-full h-full select-none" style={{ display: "flex", flexDirection: "column", position: "relative", overflow: "hidden" }}>
      ...
      {/* Header */}
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "6px 10px 4px", borderBottom: "1px solid rgba(0,240,255,0.08)", zIndex: 1, flexShrink: 0 }}>
        <span style={{ fontFamily: "monospace", fontSize: 7.5, letterSpacing: "0.2em", color: "rgba(0,240,255,0.4)" }}>
          SYS.CHRONO // ATM.SENSOR
        </span>
        <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
          <span style={{ fontFamily: "monospace", fontSize: 7, color: "rgba(0,240,255,0.3)" }}>v2.038</span>
          <span style={{ width: 5, height: 5, borderRadius: "50%", background: "#00F0FF", boxShadow: "0 0 6px #00F0FF", animation: "chrono-blink 2s ease-in-out infinite" }} />
        </div>
      </div>

      {/* TIME section */}
      <div style={{ padding: "10px 12px 8px", zIndex: 1, flexShrink: 0, position: "relative" }}>
        <Brackets color="#00F0FF" size={8} opacity={0.3} />
        <div onClick={toggle24h} title={is24h ? "Switch to 12h" : "Switch to 24h"}
          style={{ display: "flex", alignItems: "baseline", justifyContent: "center", gap: 2, cursor: "pointer", paddingTop: 4 }}>
          <span style={{ fontFamily: "monospace", fontWeight: 800, fontSize: 36, color: "#00F0FF", textShadow: "0 0 20px rgba(0,240,255,0.6), 0 0 40px rgba(0,240,255,0.2)", letterSpacing: "-0.02em", lineHeight: 1 }}>
            {hh}:{min}
          </span>
          {!is24h && <span style={{ fontFamily: "monospace", fontSize: 10, color: "rgba(0,240,255,0.5)", marginLeft: 3, fontWeight: 700 }}>{ampm}</span>}
        </div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 4, paddingLeft: 4 }}>
          <div style={{ position: "relative", width: 64, height: 64, flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center" }}>
            <SecondsArc seconds={ss} />
            <span style={{ fontFamily: "monospace", fontSize: 13, fontWeight: 700, color: "rgba(0,240,255,0.7)", zIndex: 1 }}>:{sec}</span>
          </div>
          <div style={{ flex: 1 }}>
            <div style={{ height: 2, background: "rgba(0,240,255,0.08)", borderRadius: 2, marginBottom: 6, overflow: "hidden" }}>
              <div style={{ height: "100%", width: `${minPct * 100}%`, background: "linear-gradient(90deg, rgba(0,240,255,0.4), #00F0FF)", borderRadius: 2, boxShadow: "0 0 6px rgba(0,240,255,0.5)", transition: "width 1s linear" }} />
            </div>
            <div style={{ fontFamily: "monospace", fontSize: 8.5, color: "rgba(255,255,255,0.4)", marginBottom: 2 }}>{dayStr} · {dateStr}</div>
            <div style={{ fontFamily: "monospace", fontSize: 7.5, color: "rgba(0,240,255,0.3)" }}>{tzStr}</div>
          </div>
        </div>
      ...
      {/* Divider */}
      <div style={{ margin: "0 8px", height: 1, background: "linear-gradient(90deg, transparent, rgba(0,240,255,0.2) 30%, rgba(0,240,255,0.2) 70%, transparent)", flexShrink: 0, zIndex: 1, position: "relative" }}>
        <span style={{ position: "absolute", left: "50%", top: "50%", transform: "translate(-50%,-50%)", background: "rgba(6,8,14,0.9)", padding: "0 6px", fontFamily: "monospace", fontSize: 7, letterSpacing: "0.2em", color: "rgba(0,240,255,0.35)", whiteSpace: "nowrap" }}>
          ── ATM.SENSOR ──
        </span>
      </div>

      {/* WEATHER section — relative container for forecast overlay */}
      <div style={{ flex: 1, position: "relative", overflow: "hidden" }}>

        {/* Base weather panel */}
        <div style={{ padding: "8px 12px 10px", zIndex: 1, position: "relative", display: "flex", flexDirection: "column", justifyContent: "space-between", height: "100%" }}>
          <Brackets color={cond?.color || "#00F0FF"} size={7} opacity={0.2} thickness={1} />

          <AnimatePresence mode="wait">
            {searching ? (
              <motion.div key="search" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}
                style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                <div style={{ fontFamily: "monospace", fontSize: 8, color: "rgba(0,240,255,0.5)", letterSpacing: "0.15em" }}>LOCATION.INPUT ▸</div>
                <input ref={inputRef} autoFocus type="text" value={searchInput}
                  ...
                  style={{ width: "100%", height: 26, borderRadius: 6, padding: "0 8px", background: "rgba(0,240,255,0.05)", border: "1px solid rgba(0,240,255,0.3)", outline: "none", color: "#00F0FF", fontSize: 10, fontFamily: "monospace", caretColor: "#00F0FF" }}
                />
                {searchErr && <div style={{ fontSize: 8, color: "#FF003C", fontFamily: "monospace", letterSpacing: "0.1em" }}>⚠ {searchErr}</div>}
                <div style={{ display: "flex", gap: 5 }}>
                  {[["EXECUTE", handleSearch, "#00F0FF", "rgba(0,240,255,0.08)", "rgba(0,240,255,0.25)"],
                    ["CANCEL", () => setSearching(false), "rgba(255,255,255,0.4)", "rgba(255,255,255,0.04)", "rgba(255,255,255,0.1)"]
                  ].map(([label, fn, color, bg, bdr]) => (
                    <button key={label} onClick={fn} style={{ flex: 1, height: 22, borderRadius: 5, background: bg, border: `1px solid ${bdr}`, color, fontSize: 8.5, fontFamily: "monospace", letterSpacing: "0.1em", cursor: "pointer" }}>{label}</button>
                  ))}
                </div>
              </motion.div>
              ...
              <motion.div key="data" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}
                ...
                {/* Tap to open forecast — condition row */}
                <div
                  onClick={() => weather?.hourly?.length && setShowForecast(true)}
                  title={weather?.hourly?.length ? "Tap for 5-day hourly forecast" : undefined}
                  style={{ display: "flex", alignItems: "center", gap: 10, cursor: weather?.hourly?.length ? "pointer" : "default" }}
                >
                  <div style={{ position: "relative", flexShrink: 0 }}>
                    <i className={`fa-solid ${cond.icon}`} style={{ fontSize: 28, color: cond.color, filter: `drop-shadow(0 0 10px ${cond.color}90)` }} />
                  ...
                  <div>
                    <div style={{ fontFamily: "monospace", fontWeight: 800, fontSize: 26, color: "#fff", textShadow: `0 0 16px ${cond.color}60`, lineHeight: 1 }}>
                    ...
                    <div style={{ fontFamily: "monospace", fontSize: 9, color: "rgba(255,255,255,0.5)", marginTop: 2 }}>
                      ...
                      {weather?.hourly?.length > 0 && (
                        <span style={{ marginLeft: 5, color: "rgba(0,240,255,0.4)", fontSize: 7.5 }}>▸ FORECAST</span>
                      )}
                    </div>
                  </div>
                </div>
                ...
                {/* Data grid */}
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "4px 6px", padding: "6px 8px", background: "rgba(0,240,255,0.03)", border: "1px solid rgba(0,240,255,0.08)", borderRadius: 8 }}>
                  {[
                    ["FEELS", `${weather.feels}°C`, "#00F0FF"],
                    ["WIND",  `${weather.wind}km/h`, "#39FF14"],
                    ["HUMID", weather.humidity != null ? `${weather.humidity}%` : "N/A", "#A78BFA"],
                  ].map(([label, val, col]) => (
                    <div key={label} style={{ textAlign: "center" }}>
                      <div style={{ fontFamily: "monospace", fontSize: 7, color: "rgba(255,255,255,0.25)", letterSpacing: "0.12em" }}>{label}</div>
                      <div style={{ fontFamily: "monospace", fontSize: 11, fontWeight: 700, color: col, marginTop: 1 }}>{val}</div>
                    </div>
                  ))}
                </div>
                ...
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                  ...
                  <div style={{ display: "flex", gap: 4, alignItems: "center" }}>
                    ...
                    <div style={{ fontFamily: "monospace", fontSize: 7, letterSpacing: "0.12em", color: "#39FF14", padding: "1px 5px", border: "1px solid rgba(57,255,20,0.25)", borderRadius: 4, background: "rgba(57,255,20,0.07)" }}>
                      NOMINAL
                    </div>
                  </div>
                </div>
              ...
              <motion.div key="loading" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                ...
                <div style={{ fontFamily: "monospace", fontSize: 8, color: "rgba(0,240,255,0.35)", letterSpacing: "0.2em" }}>
                  {wxStatus === "loading" ? "SCANNING ATMOSPHERE…" : "SENSOR OFFLINE"}
                </div>
                {wxStatus === "error" && (
                  <button onClick={() => setSearching(true)} style={{ fontFamily: "monospace", fontSize: 8, color: "rgba(0,240,255,0.5)", background: "transparent", border: "1px solid rgba(0,240,255,0.2)", borderRadius: 5, padding: "3px 8px", cursor: "pointer" }}>
                    SET LOCATION
                  </button>
                )}
              </motion.div>
        ...
        {/* Forecast overlay — slides over the weather section */}
        <AnimatePresence>
          {showForecast && (
            <ForecastStrip
