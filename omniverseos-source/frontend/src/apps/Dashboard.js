...
export default function Dashboard() {
  ...
  return (
    <div className="w-full h-full overflow-y-auto p-4 sm:p-6 text-white" data-testid="dashboard-app">
      {/* Header row */}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
        {[
          ...
          <div key={s.label} className="glass-light rounded-xl p-4">
            ...
            <div className="font-heading text-xl sm:text-2xl font-bold">{s.v}</div>
          </div>
        ))}
      </div>

      {/* Charts + quick launch — col on mobile, 3-col on desktop */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="md:col-span-2 glass-light rounded-xl p-5">
            ...
            {quick.map((id) => {
              ...
              return (
                <button
                  ...
                  onClick={() => openApp(id)}
                  className="aspect-square rounded-lg flex flex-col items-center justify-center gap-1 hover:bg-white/10 active:bg-white/10 transition border border-white/5"
                  style={{ minHeight: 44 }}
                >
      ...
      {/* Bottom row — col on mobile, 3-col on desktop */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
        <div className="glass-light rounded-xl p-5 md:col-span-2">
        ...
        <div className="glass-light rounded-xl p-5">
          ...
          <div className="mt-3 h-1.5 bg-white/5 rounded-full overflow-hidden">
            <div
              ...
              style={{ width: `${stats.tasks ? (stats.tasks_done / stats.tasks) * 100 : 0}%` }}
            />
          </div>
        </div>
      </div>
