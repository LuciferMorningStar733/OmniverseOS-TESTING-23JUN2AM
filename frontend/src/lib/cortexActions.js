...
function parseDateString(str) {
  ...
  if (s.startsWith("next ")) {
    ...
    if (idx !== -1) { const d = new Date(today); const diff = (idx - d.getDay() + 7) % 7 || 7; d.setDate(d.getDate() + diff); return d.toISOString().slice(0, 10); }
  }
  const mD = s.match(/([a-z]+)\s+(\d{1,2})(?:st|nd|rd|th)?/);
  if (mD && MONTHS[mD[1]] !== undefined) return new Date(today.getFullYear(), MONTHS[mD[1]], parseInt(mD[2])).toISOString().slice(0, 10);
  const dM = s.match(/(\d{1,2})(?:st|nd|rd|th)?\s+([a-z]+)/);
  if (dM && MONTHS[dM[2]] !== undefined) return new Date(today.getFullYear(), MONTHS[dM[2]], parseInt(dM[1])).toISOString().slice(0, 10);
...
function extractFromSegment(seg, context = {}) {
  ...
  if (openM) { const resolved = resolveTarget(openM[1]); if (resolved) actions.push(resolved); }

  const closeM = s.match(/(?:close|exit|quit|hide|kill)\s+(?:the\s+)?(.+?)(?:\s*$|\s*,|\s+and\b|\s+then\b)/);
  if (closeM) { const t = closeM[1].trim().replace(/\s+/g, " "); if (INTERNAL_APP_MAP[t]) actions.push({ type: "close_app", appId: INTERNAL_APP_MAP[t], label: t }); }

  const minM = s.match(/(?:minimize|minimise|collapse|hide in taskbar)\s+(?:the\s+)?(.+?)(?:\s*$|\s*,|\s+and\b|\s+then\b)/);
  if (minM) { const t = minM[1].trim().replace(/\s+/g, " "); if (INTERNAL_APP_MAP[t]) actions.push({ type: "minimize_app", appId: INTERNAL_APP_MAP[t], label: t }); }

  const focusM = s.match(/(?:switch to|focus|activate|return to|jump to)\s+(?:the\s+)?(.+?)(?:\s*$|\s*,|\s+and\b|\s+then\b)/);
  if (focusM) { const t = focusM[1].trim().replace(/\s+/g, " "); if (INTERNAL_APP_MAP[t]) actions.push({ type: "focus_app", appId: INTERNAL_APP_MAP[t], label: t }); }
  ...
  if (sm1 && SEARCH_ENGINES[sm1[1].toLowerCase().trim()]) {
    ...
    const builder = (eng && SEARCH_ENGINES[eng]) ? SEARCH_ENGINES[eng] : (context.lastUrl ? (SEARCH_ENGINES[engineFromUrl(context.lastUrl)] || SEARCH_ENGINES.google) : SEARCH_ENGINES.google);
    actions.push({ type: "open_url", url: builder(q), label: `Search: ${q}` });
  }
  if (pm) actions.push({ type: "open_url", url: SEARCH_ENGINES.youtube(pm[1].trim()), label: `Play: ${pm[1]}` });
  ...
  if (calWithTime) {
    ...
    if (date) { const title = calWithTime[3].trim(); actions.push({ type: "add_event", title, date, time: parseTimeString(calWithTime[2].trim()), label: `Event: ${title}` }); }
  } else if (calPlain) {
    const date = parseDateString(calPlain[1].trim());
    if (date) { const title = calPlain[2].trim(); actions.push({ type: "add_event", title, date, time: "09:00", label: `Event: ${title}` }); }
  }

  const addTaskRe = /add\s+(?:"([^"]+)"|(.+?))\s+to\s+(?:my\s+)?(?:todo|tasks?|to-do|to do|roadmap|task list)(?:\s*$)/;
  ...
  if (createM && !addTaskM) { const title = (createM[1] || createM[2]).trim(); actions.push({ type: "add_task", title, label: `Task: ${title}` }); }

  const completeM = s.match(/(?:mark|set|complete|finish|check off)\s+(?:"([^"]+)"|(.+?))\s+(?:as\s+)?(?:complete|done|finished)(?:\s*$)/);
  if (completeM) { const title = (completeM[1] || completeM[2]).trim(); actions.push({ type: "complete_task", title, label: `Done: ${title}` }); }
  ...
  if (delTaskM && !s.includes("event") && !calWithTime && !calPlain) {
    ...
    if (title.length > 1) actions.push({ type: "delete_task", title, label: `Delete task: ${title}` });
  }
    ...
    try {
      switch (action.type) {
        ...
        case "close_app": {
          ...
          if (win && closeWindow) { closeWindow(win.id); results.push({ action, success: true }); }
          else results.push({ action, success: false, error: `${action.appId} is not open` });
          break;
        }
        case "minimize_app": {
          const win = windows.find(w => w.app === action.appId && !w.minimized);
          if (win && minimize) { minimize(win.id); results.push({ action, success: true }); }
          else results.push({ action, success: false, error: `${action.appId} is not open or already minimized` });
          break;
        }
        case "focus_app": {
          const win = windows.find(w => w.app === action.appId);
          if (win && focusWindow) { focusWindow(win.id); results.push({ action, success: true }); }
          else if (openApp) { openApp(action.appId); results.push({ action, success: true }); }
          else results.push({ action, success: false, error: "Window manager unavailable" });
          break;
        }
        case "add_task":
          await crud("tasks").create({ title: action.title, status: "todo", priority: "medium", description: "" });
          results.push({ action, success: true });
          break;
        case "complete_task": {
          const tasks = await crud("tasks").list();
          const t = fuzzyMatch(tasks, action.title);
          if (t) { await crud("tasks").update(t.id, { ...t, status: "done" }); results.push({ action: { ...action, title: t.title }, success: true }); }
          else results.push({ action, success: false, error: `Task "${action.title}" not found` });
          break;
        }
        case "delete_task": {
          const tasks = await crud("tasks").list();
          const t = fuzzyMatch(tasks, action.title);
          if (t) { await crud("tasks").remove(t.id); results.push({ action: { ...action, title: t.title }, success: true }); }
          else results.push({ action, success: false, error: `Task "${action.title}" not found` });
          break;
        }
        case "add_event":
          await crud("events").create({ title: action.title, date: action.date, time: action.time || "09:00", color: "#00F0FF", description: "" });
          results.push({ action, success: true });
          break;
        case "delete_event": {
          const events = await crud("events").list();
          const ev = fuzzyMatch(events, action.title);
          if (ev) { await crud("events").remove(ev.id); results.push({ action: { ...action, title: ev.title }, success: true }); }
          else results.push({ action, success: false, error: `Event "${action.title}" not found` });
          break;
        }
        default:
